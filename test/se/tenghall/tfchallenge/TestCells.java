...
public class TestCells {
	...
	public void shouldBeEmptyByDefault() throws Exception {
		assertEquals("", sheet.getCellContents("A1"));
		assertEquals("", sheet.getCellContents("ZX347"));
	}
	...
	public void shouldStorText() throws Exception {
		...
		assertEquals(aTestString1, sheet.getCellContents(A_CELL));

		sheet.put(A_CELL, aTestString2);
		assertEquals(aTestString2, sheet.getCellContents(A_CELL));

		sheet.put(A_CELL, "");
		assertEquals("", sheet.getCellContents(A_CELL));
	...
	public void shouldHandleManyCells() throws Exception {
		...
		assertEquals(cellOne, cellContents_1, sheet.getCellContents(cellOne));
		assertEquals(cellTwo, cellContents_2, sheet.getCellContents(cellTwo));
		assertEquals(cellThree, cellContents_3,
				sheet.getCellContents(cellThree));

		sheet.put(cellOne, cellContents_4);
		assertEquals(cellOne + " contains new value", cellContents_4,
				sheet.getCellContents(cellOne));
		assertEquals(cellTwo + " contains same value", cellContents_2,
				sheet.getCellContents(cellTwo));
	...
	public void shouldBeAbleToHandNumericalValues() throws Exception {
		...
		assertEquals("Obvious string", "X99", sheet.getCellContents(A_CELL));

		sheet.put(A_CELL, "14");
		assertEquals("Obvious number", "14", sheet.getCellContents(A_CELL));

		sheet.put(A_CELL, " ");
		assertEquals("Just a blank", " ", sheet.getCellContents(A_CELL));

		sheet.put(A_CELL, " 99 X");
		assertEquals("Whole string must be numeric", " 99 X",
				sheet.getCellContents(A_CELL));

		sheet.put(A_CELL, " 1234 ");
		assertEquals("Ignore blanks", "1234", sheet.getCellContents(A_CELL));
	}
	...
	public void shouHandleFormulaSpec() throws Exception {
		...
		assertEquals("Not a formula", " =7", sheet.getCellContents("B1"));
	...
	public void shouldHandleConstantFunctions() throws Exception {
		...
		assertEquals("Value", "7", sheet.getCellContents(A_CELL));
	...
	public void testParentheses() throws Exception {
		sheet.put(A_CELL, "=(7)");
		assertEquals("Parends", "7", sheet.getCellContents(A_CELL));
	}

	@Test
	public void testDeepParentheses() throws Exception {
		sheet.put(A_CELL, "=((((10))))");
		assertEquals("Parends", "10", sheet.getCellContents(A_CELL));
	}
}
